...
foreach ($new_list as $i => &$item) {
    ...
    if ($level == $start_level) {
        $nav_item .= "\n" . TplL2BHelper::tab($level * $tab_multiplier + 1) . '<li'.$class.'>' . "\n" . TplL2BHelper::tab($level * $tab_multiplier + 2);
    } else {
        $nav_item .= "\n" . TplL2BHelper::tab($level * $tab_multiplier + 1) . '<span role="listitem"'.$class.'>' . "\n" . TplL2BHelper::tab($level * $tab_multiplier + 2) . '<span data-hidden="if-css">&nbsp;&nbsp;&nbsp;</span>';
    }
    ...
    if ($next_item && $next_level > $level) {
        $nav_item .= "\n" . TplL2BHelper::tab($level + 2) . '<div class="dropdown  dropdown--only-wide" data-js="dropdown">';
        ...
        $nav_item .= "\n" . TplL2BHelper::tab($level + 3) . '<div class="dropdown__area" id="' . $item->alias . '-sub-menu--target">';
        $nav_item .= "\n" . TplL2BHelper::tab($level + 4) . '<div role="list" class="sub-nav__items  sub-nav__items--stacked  d-background--dark">';
    }
    // The next item is shallower.
    elseif ($next_item && $next_level < $level) {
        $nav_item .= "\n" . TplL2BHelper::tab($level + 4) . '</span>';
        $nav_item .= "\n" . TplL2BHelper::tab($level + 3) . '</div>';
        $nav_item .= "\n" . TplL2BHelper::tab($level + 2) . '</div>';
        $nav_item .= "\n" . TplL2BHelper::tab($level + 1) . '</div>';
        $nav_item .= "\n" . TplL2BHelper::tab($level) . '</li>';
    }
    // The next item is on the same level.
    else {
        if ($level == $start_level) {
            $nav_item .= "\n" . TplL2BHelper::tab($level * $tab_multiplier + 1) . '</li>';
        } else {
            $nav_item .= "\n" . TplL2BHelper::tab($level * $tab_multiplier + 1) . '</span>';
        }
    }
    $nav .= $nav_item;
}
...
<?php echo TplL2BHelper::tab(2); ?></ul>
<?php endif; ?>
<?php endif; ?>