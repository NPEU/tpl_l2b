...
// Determine tracking level
$isGuest = in_array($guestGroupId, $userGroups);
$isMember = in_array($memberGroupId, $userGroups);
...
if (!isset($menu_id)) {
    $menu_id = TplL2BHelper::get_menu_id();
}
...
// Brand
//$page_brand        = TplL2BHelper::get_brand();
...
// Template
$page_template        = TplL2BHelper::get_template();
...
$pageclass = $menu_item_params->get('pageclass_sfx', '');
...
foreach ($sidebar_bottom_modules as $module) {
    ...
    if ($module_params->get('layout') == 'l2b:Area-Menu') {
        $t1 = HTMLHelper::_('content.prepare', '{loadmoduleid ' . $module->id .'}');

        if (!empty($t1)) {
            $page_has_area_menu = true;
            $page_area_menu_id  = TplL2BHelper::html_id($module->title);
            continue;
        }
    }

    if ($module_params->get('layout') == 'l2b:Section-Menu') {
...
#echo '<pre>'; var_dump($page_head_data); echo '</pre>'; exit;
// Assets:
$page_stylesheets = TplL2BHelper::remove_joomla_stylesheets($page_head_data['styleSheets'], $doc);
...
$doc->joomla_scripts = [];
$page_scripts     = TplL2BHelper::remove_joomla_scripts($page_head_data['scripts'], $doc);
...
$page_hero         = (object) $menu_item_params->get('hero_image');
$page_has_hero = false;


if (!empty($page_hero)) {
    #echo '<pre>'; var_dump($page_hero); echo '</pre>'; exit;
    if (!empty($page_hero->image) && (bool) $page_hero->enabled == true) {


        // Tidy Joomla image src fragment:
        $page_hero->image = preg_replace('/#.*$/', '', $page_hero->image);
...
if (!empty($all_page_heroes)) {
    foreach ($all_page_heroes as $image) {
        ...
        if (isset($image_meta['copyright'])) {
            $image->credit = trim(TplL2BHelper::tweak_markdown_output(
...
/*$page_footer_text = TplL2BHelper::tweak_markdown_output(
...
/*if (!empty($doc->article->headline_image['headline-image'])) {
    $data = TplL2BHelper::resolve_image_data($doc->article->headline_image['headline-image']);
    $doc->article->headline_image['headline-image'] = TplL2BHelper::resolve_image_path($data['imagefile']);
}*/
...
// Nested Layouts:
$inner_structure = 'structure--branded';
